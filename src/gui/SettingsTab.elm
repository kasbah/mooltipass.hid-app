...
content (w,h) =
    let userNameBox = stringEntry (w,50) "Name" "Bob"
        passwordBox = stringEntry (w,50) "Password" "abc123"
        content = container w h midTop <| flow down
            [ userNameBox
            , passwordBox
            ]
{-
        exitButton       = button (send commonActions EndMemManage) "exit"
        showMem infodata = container w h midTop <| flow down
            [ favorites w infodata
            , spacer 1 heights.settingsSpacer
            , credentials
                ( w
                , h - (heightOf (favorites w infodata))
                    - heights.settingsSpacer
                    - (heights.button + 4)
                    - heights.settingsSpacer)
                infodata
            , spacer 1 heights.settingsSpacer
        ...
        modeExited    = leftAligned <| whiteText "memory settingsment mode exited"
        reEnter       =
        ...
        pleaseAccept = leftAligned
            <| whiteText "please accept memory settingsment mode on the device"
        working = leftAligned
            <| whiteText "working..."
    in case info of
        NoMemInfo             -> reEnter
...
stringEntry : (Int, Int) -> String -> String -> Element
stringEntry (w,h) str cur =
    let content = leftAligned <| whiteText cur
    in  box (w,h) str <| content
...
favorites w info =
    let style =
        ...
        ch = heights.settingsLogin * 5 + (5*6)
        l = Debug.log "info" info
    in box (w, ch + 20 + heights.settingsTitle) "Favorites"
        <| flow down
            [ spacer 1 10
...
credentials (w,h) i =
    let ht = heights.settingsTitle
        ...
        txt s = container w heights.settingsTitle middle
